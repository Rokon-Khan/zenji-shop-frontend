...
import wordmark from "@/assets/zenji-wordmark.png.asset.json";
      ...
      <div className="border-b border-border bg-ink">
        <div className="mx-auto grid h-16 max-w-[1600px] grid-cols-[auto_1fr_auto] items-center gap-4 px-4 sm:px-6">
          ...
          <nav className="hidden items-center justify-center gap-7 lg:flex">
            {NAV.map((n) => (
              <Link
                ...
                className="label-xs font-bold text-foreground transition-colors hover:text-primary"
              >
                {n.label}
              </Link>
            ))}
            ...
            >
              ...
              {more && (
                <div className="absolute left-1/2 w-40 -translate-x-1/2 border border-border bg-card p-2">
                  {MORE.map((m) => (
                    <Link
                      key={m.to}
                      to={m.to}
          ...
          <div className="flex items-center justify-end gap-3">
            ...
            <Link to="/login" aria-label="Account" className="hover:text-primary">
